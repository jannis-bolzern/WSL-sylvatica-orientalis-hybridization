...
# ---- Templates
TEMPLATE_BURNIN_INI <- "ini_files/templates/beech1_burnin.ini"
TEMPLATE_RUN_INI    <- "ini_files/templates/beech1.ini"
...
for (d in OUT) dir.create(d, showWarnings = FALSE, recursive = TRUE)
...
DIRS <- list(
  ...
  burnin   = file.path(OUT$ini, "burnin"),
  run      = file.path(OUT$ini, "run")
)
...
PLOT_REP_DIR <- file.path(DIRS$plots)
...
for (i in seq_len(nrow(manifest))) {
  ...
  burn_ini_path <- file.path(DIRS$burnin, paste0(sim, "_burnin.ini"))
  ...
  run_ini_path <- file.path(DIRS$run, paste0(sim, ".ini"))
...
message("Wrote burn-in ini files to: ", DIRS$burnin)
message("Wrote run ini files to:    ", DIRS$run)
...
```{bash}
./nemoage ini_files/burnin/sc_p05_dispersed_rep01_burnin.ini
./nemoage ini_files/run/sc_p05_dispersed_rep01.ini
```
