...
PLOT_DIR <- file.path(PROJECT_ROOT, "plots_layouts")
...
find_quanti_file <- function(res_root, cfg, pct, run_id, gen, rep = 1) {
  sim <- sprintf("%s_p%02d_r%02d", cfg, pct, run_id)
  gen_tag <- sprintf("%03d", gen)
  rep_tag <- sprintf("%02d", rep)
  
  cand <- file.path(
    res_root, cfg, sprintf("p%02d", pct), sprintf("r%02d", run_id),
    "run", "quanti",
  ...
  hits2 <- list.files(res_root, pattern = rx, recursive = TRUE, full.names = TRUE)
...
for (gen in TIMES) {
  ...
  p1_dt_all <- rbindlist(lapply(CONFIGS, function(cfg) {
    rbindlist(lapply(PCTS, function(pct) {
      ...
      qfile <- find_quanti_file(RES_ROOT, cfg, pct, RUN_ID, gen, rep = SHOW_REP)
      if (is.na(qfile)) {
  ...
  p_p1 <- ggplot(map_dt, aes(x = x_plot, y = y_plot)) +
    ...
    geom_segment(
      data = stats_dt,
      aes(x = 0.5, xend = N_side + 0.5, y = 0.5, yend = 0.5),
      inherit.aes = FALSE,
      color = "grey85",
      linewidth = 0.3
    ) +
