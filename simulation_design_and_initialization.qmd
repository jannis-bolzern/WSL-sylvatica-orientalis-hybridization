...
This script define selection scenarios, and create a helper that add or remove selections from the ini file (created later). The selection matrix is row = N stages under selection, column = trait under selection, value = strength of selection (variance).
...
Running all simulations in parallel (GNU parallel, adjust -j for number of cores):

``` bash
mkdir -p logs
parallel -j "$(nproc)" --halt soon,fail=1 --bar \
  './nemoage {} > logs/{/.}.log 2>&1' ::: ini_files/run/*_k30_b0.03.ini
```
