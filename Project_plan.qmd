...
### run anova test to check significance (if needed)
...
# the output is a unique file : Quanti_data_Allenwiller_year2020.RDS
res_path2 <- "C:/Users/stefanin/Downloads/quanti"
# res_path2 <- "C:/Users/stefanin/Downloads/WSL-sylvatica-orientalis-hybridization-main/WSL-sylvatica-orientalis-hybridization-main"
...
### Overall hybrid proportions

Then we define a general **hybrid metric which includes all the hybrid classes (F1, F2, BC),** i.e. everything that is not pure. We check the proportion of overall hybrids across the simulation and per patch:

```{r}
#| label: overall hybrids proportions across years
...
# plot the proportion of hybrids + SD for each stage across years
hyb_summary <- hyb_prop[, .(
  mean_prop = mean(prop_hybrids),
  sd_prop   = sd(prop_hybrids)
),
by = .(configuration, ori_proportion, selection, generation, stage, k, b)]

ggplot(hyb_summary,
       aes(generation, mean_prop, colour = factor(stage))) +
  geom_line() +
  geom_ribbon(aes(ymin = mean_prop - sd_prop,
              ...
              alpha = 0.25,
              colour = NA) +
  facet_grid(configuration + b ~ k + ori_proportion) +
  theme_bw()
...
To compare the proportion of all hybrids across the designs, for one specific year:
...
## maybe to adjust
# generation = 100
ggplot(subset(hyb_summary,generation == 100), 
...
NB: this allows to **quantify the difference between scenarios in terms of presence of hybrids.**

The problem of grouping all the hybrids is that we cannot distinguish F1 from the later generation hybrids, while the problem of summarizing P1 across the landscape is that we cannot distinguish patches with 50% Pure orientalis + 50% pure sylvatica from 100% F1 (for instance).
