...
2.  Perform a Sensitivity analysis on the influence of the following parameters on the simulation, using one simulation per set (empty / beech / mixed) and few replicates (5) and few generations (like 50 or 100) to speed up:

    -   carrying capacity
    -   competition coefficient
    -   selection ?

    If there is no or negligible difference in the hybridization patterns, just choose one value.

3.  Perform all the simulations with the chosen values.
...
-   `patch_nbfem` homogeneous\
        → seedlings can grow everywhere
```

-   **Harvesting / clearing**

    -   `extinction_rate {{0, 0, 0, 1 ...}}`\
        → value = 1 in patches that are cut or reserved for *Orientalis*
...
### 3. Starting from mature European beech forest (40 years)
...
-   carrying capacity
-   competition coefficient
-   selection? fitness parameters?

2.  Full simulations:

-   one value per parameter if possible
...
1.  population structure, age of reproduction and stage survival (from COMPADRE):
    ...
    Survival from COMPADRE + cit. previous paper, with adults producing 1000 seedlings per generation.

2.  patch number and grid: 25 x 25 grid, each patch is 16\^2 m.

3.  carrying capacity –-\> *patch_nbfem* (max number of adult females per patch, can encode planting, thinning, harvesting or unsuitable habitat)
...
3.  carrying capacity? –-\> *patch_nbfem*
4.  competition coefficient —\> three b parameters?
5.  fitness reduction / selection (DMI trait) —\> three DMI parameters?
