...
To test the effect of k and b on the response variable hybrid proportions, I fit one model per subset simulation (configuration x ori proportion):
...
# split dataset
hyb_list <- split(hyb_prop, list(hyb_prop$configuration, hyb_prop$ori_proportion), drop = TRUE)

# loop to fit the model 
library(lme4)

results_list <- list()

for(i in seq_along(hyb_list)) {
  
  dat <- hyb_list[[i]]
  
  model <- glmer(
    cbind(N_hybrid, N - N_hybrid) ~ k + b + (1 | replicate),  ## comparing N hybrids ~ N not hybrids
    data = dat,
    family = binomial  ## N_hyb ~ binomial(N, p) --?
  )
  
  results_list[[names(hyb_list)[i]]] <- model
}


## create summary of results for each model of each sim
...
res_coef <- lapply(results_list,model_summary)


## store in one df
res_df <- do.call(rbind, lapply(names(res_coef), function(nm) {
  coefs <- res_coef[[nm]]
  cfg <- strsplit(nm, "\\.")[[1]]
  data.frame(
    configuration = cfg[1],
    ori_proportion = as.numeric(cfg[2]),
    term = rownames(coefs),
    estimate = coefs[,1],
    std_error = coefs[,2],
    z_value = coefs[,3],
