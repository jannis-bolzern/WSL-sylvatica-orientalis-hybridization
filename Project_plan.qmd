...
NB: to parametrise the scenario, start from the number of adults in a 1 ha plot and the number of seedlings reported: 
-  Pauline reported a max basal area of approx 0.2-0.3 m^2 per tree, ---> use to calcaulte the b coefficient
-  Allenwiller = 68 adults per ha, Waldi = 34 adults per ha
-   Gratzler et al., 2022 reported about 630 beech seedlings in 1 ha plot
...
Case 1: selection against E. beech ---> directional selection implemented by stabilizing selection with the optimum at + 1 (O. beech), with fitness declined smoothly toward P1 (z) = -1 (hybrids have an intermediate value). The selection_fitness_model set the fitness of the individual independent from the fitness of the other individuals of the population (--> possible population decline, stronger eco-evo feedback). Selection variance (w^2) is the quadratic of the inverse of the selection strenght, therefore, the smaller, the stronger the selection, the larger, the weaker the selection. 
If 
...
regulation      4
save_stats      5  # census
aging_multi     6  # stage transition (after disperse, to save data for both adults and offspring)
save_files      7  # write summary stats to file

## SELECTION ##
viability_selection        3          ## rank in life cycle
selection_trait            quant
selection_model            quadratic
selection_fitness_model    absolute
selection_at_stage         {{0}}       ## seedlings only or {{0,1}} for seedlings up to 4 age

selection_local_optima     {{1.0}}     ## optimum = pure Oriental
selection_variance         {{20}}     ## strength of selection

Case 2. selection against O. beech

The same as Case 1 but 

selection_local_optima     {{-1.0}}    ## optimum = pure European
selection_variance         {{20}}

Potential Case 3. selection against both pure species = hybrid advantage

selection_local_optima     {{0.0}}     ## hybrids optimal
selection_variance         {{20}}
...
--> are the results consistent? E.g. is the hybrid proportion consistently highest(lowest) in the same configuration x ori_proportion across all k and b values, or is it changing? 
If the main results are changing for different K and B values, the significantly affect the hybridization. 
